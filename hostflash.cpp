...
static bus::match::match hiomap_match_properties(struct hostflash *ctx)
{
    ...
    bus::match::match match(*ctx->bus, properties,
            std::bind(hiomap_handle_property_update, ctx,
                      std::placeholders::_1));
...
static bus::match::match hiomap_match_signal_v2(struct hostflash *ctx, const char *name)
{
    using namespace bus::match;

    auto signals = rules::type::signal()
                      + rules::path(HIOMAPD_OBJECT)
                      + rules::interface(HIOMAPD_IFACE_V2)
                      + rules::member(name);
...
static ipmi_ret_t flash_command_create_window(struct hostflash *ctx, bool ro,
...
static ipmi_ret_t flash_command_create_read_window(ipmi_request_t request,
                                                   ipmi_response_t response,
                                                   ipmi_data_len_t data_len,
                                                   ipmi_context_t context)
{
    ...
    return flash_command_create_window(ctx, true, request, response, data_len);
}

static ipmi_ret_t flash_command_create_write_window(ipmi_request_t request,
                                                    ipmi_response_t response,
                                                    ipmi_data_len_t data_len,
                                                    ipmi_context_t context)
{
...
static ipmi_ret_t flash_command_flush(ipmi_request_t request,
                                      ipmi_response_t response,
                                      ipmi_data_len_t data_len,
                                      ipmi_context_t context)
{
...
static ipmi_ret_t ipmi_openpower_flash_command(ipmi_netfn_t netfn,
                                               ipmi_cmd_t cmd,
                                               ipmi_request_t request,
                                               ipmi_response_t response,
                                               ipmi_data_len_t data_len,
                                               ipmi_context_t context)
{
    ...
    ipmi_ret_t cc = flash_commands[flash_cmd](flash_req, flash_resp,
                                              &flash_len, context);
    if (cc != IPMI_CC_OK) {
        *data_len = 0;
        return cc;
...
static void register_openpower_flash_commands()
{
    ...
    struct openpower::flash::hostflash *ctx =
        new openpower::flash::hostflash();
    ...
    ctx->event_lookup["BmcReboot"] = BMC_EVENT_REBOOT;
     ...
     */
    ctx->properties = new bus::match::match(
                            std::move(hiomap_match_properties(ctx)));
